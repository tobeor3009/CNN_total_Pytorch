...
class AxialScan(torch.utils.data.Dataset):
    def __init__(self,
        ...
        self.dataset = load_obj("../Dataset/Asan_Brain_CT/train/png/normal/dataset.pkl")
        ...
        if get_rank() == 0: print(f"INFO.{len(self.dataset)} patients for {split}")
    ...
    def __getitem__(self, idx):
        ...
        imgs = sorted(glob(os.path.join(query_dir, "*.png")), reverse = self.reverse)
...
class loss_fn(torch.nn.Module):
    ...
    def age_loss_fn(self, logit_age, y_age):
        age_arange = torch.arange(start=self.age_start, end=self.age_end, dtype=torch.FloatTensor).cuda() / 100
        age_arange = age_arange.unsqueeze(0).repeat((logit_age.size(0), 1))

        mean_pred = torch.einsum('bs, bs->b', logit_age, age_arange)
        var_pred = torch.einsum('bs, bs->b', logit_age, (age_arange - mean_pred.unsqueeze(1)) ** 2)
        mean_y = y_age
        # var_y  = 1

        kl_loss = 0.5 * (-torch.log(var_pred) + var_pred + (mean_pred - mean_y)**2 - 1.).mean()
        return kl_loss
...
def train(args):
    ...
    if args.is_distributed:
        ...
        synchronize()
        if get_rank() == 0: print(f"INFO.Distributed Launch with {args.n_gpu} GPUs!!!")
    ...
    my_transforms = {
        "train": transforms.Compose(
                            [
                                ...
                                transforms.RandomApply(torch.nn.ModuleList([transforms.GaussianBlur(3, sigma=(0.1, 2.0))]), p = 0.25),
    ...
    real_dataloader = {phase: DataLoader(real_dataset[phase],
                                        ...
                                        num_workers = 0,
                                        sampler     = data_sampler(real_dataset[phase], shuffle=True, distributed=args.is_distributed),
                                        drop_last   = True,
                                        pin_memory  = True) for phase in phases}

    "set dir"
    if get_rank() == 0:
        args.save_dir = os.path.join("checkpoint", "patient_info_predictor", f"{args.model}_{args.n_latent_layer}layered_transformer")
    ...
    if get_rank() == 0: print("INFO. start training...")
    for epoch in range(start_epoch, args.epochs):     
        ...
        for phase in phases:
            ...
            if get_rank(): pbar = enumerate(real_dataloader[phase])
            else:          pbar = tqdm(enumerate(real_dataloader[phase]), dynamic_ncols=True, smoothing=0.01)
            for step, (scan, label) in pbar:
                ...
                with torch.set_grad_enabled(phase == "train"):
                    pred = model(random_aug_g(scan) if phase == "train" else scan)
                ...
                if get_rank() == 0:               
                    ...
                    running_age_loss = np.array(losses[epoch][phase]["age"])[-1000:].mean()
                    running_sex_loss = np.array(losses[epoch][phase]["sex"])[-1000:].mean()

                    sex_correct += (sex == (pred_sex > 0.5)).sum()
                    sex_acc = sex_correct / n_patients

                    writer.add_scalar("age_loss", age_loss.item(), epoch * len(real_dataloader[phase]) + step)
                    writer.add_scalar("sex_loss", sex_loss.item(), epoch * len(real_dataloader[phase]) + step)
                    writer.add_scalar("sex_acc",  sex_acc, epoch * len(real_dataloader[phase]) + step)
...
def test(args):
    ...
    args.is_distributed = args.n_gpu > 1
    if args.is_distributed: model = torch.nn.DataParallel(model)
    ...
    save_dir = os.path.join("checkpoint", "patient_info_predictor", f"{args.model}_lambda({args.lambda_age},{args.lambda_sex})")
...
def anomaly_detection(args):
    ...
    class BrainCT(torch.utils.data.Dataset):
        def __init__(self, transform, reverse=True, n_slices = 32):
            ...
            for severity in severities:
                query_dirs = glob(f"../Dataset/Asan_Brain_CT/test/{split}/{DATASET_VERSION}/{severity}/*")       
        ...
        def __getitem__(self, idx):
            ...
            imgs = sorted(glob(os.path.join(query_save_dir, 'png', '*.png')), reverse = self.reverse)[:self.n_slices]
            imgs = [Image.open(img) for img in imgs]
            if len(imgs) > self.n_slices:
                start_idx = random.randint(0, len(imgs) - self.n_slices)
                end_idx = start_idx + self.n_slices
    ...
    for data in pbar:     
        ...
        if result[severity]["label"] == []:
            ...
            result[severity]["label"] = torch.cat([result[severity]["label"], label], 0)
            result[severity]["pred"] = torch.cat([result[severity]["pred"], pred], 0)
        
    for severity in severities:
        labels = result[severity]["label"]
        preds = result[severity]["pred"]
        sex_accuracy = (labels[:, 1] == (preds[:, 1] > 0.5)).sum() / labels.size(0)
        age_diff = 100 * torch.mean(preds[:, 0] - labels[:,0])
        age_l1_diff = 100 * F.l1_loss(labels[:, 0], preds[:, 0], reduction='mean')
        age_std  = (100 * (labels[:, 0] - preds[:, 0])).std()
    ...
    fname = os.path.join("checkpoint", "patient_info_predictor", f"{args.model}_lambda({args.lambda_age},{args.lambda_sex})", "anomaly_detection_result.pkl")
    save_obj(result, fname)

if __name__  == "__main__":
    parser = argparse.ArgumentParser(description='patient info', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--n_latent_layer", type=int, default = 6)
