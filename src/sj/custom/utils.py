...
def save_summary_figure(real, fake, overlay, residue, residue_bet, residue_filtered, figure_save_dir, fName, save_format = 'png'):
    ...
    residue_filtered_path = os.path.join(figure_save_dir, "residue_filtered", fname)
    array2image(np.squeeze(residue_filtered, 2), residue_filtered_path, colorize = True)
...
def load_obj(PATH, printf = False):
    ...
    if os.path.exists(PATH):
        if printf: print(f"INFO.loaded {stat_pkl}")
...
def dcm2jpg():
    ...
    for case in ['abnormal','normal','benign']:
        ...
        for brainct_dir in tqdm(brainct_dirs):
            ...
            plt.savefig(os.path.join(case_jpg_dir,f"{patient_id}.jpg"), dpi = DPI, transparent = True)
            plt.close()

            if case == "abnormal":
                mask_hdrs       = glob(os.path.join(brainct_dir, 'Mask_DataSet', "*.hdr"))
                ...
                for i, (brain_img, target) in enumerate(zip(brain_imgs,targets)):
                    fig.add_subplot(rows_, columns_, i + 1)
                    plt.imshow(brain_img)
                    plt.imshow(np.flip(target, 0).astype(bool),alpha = 0.7, cmap = plt.cm.inferno)
                plt.axis('off')
                plt.axis("tight")

                plt.savefig(os.path.join(case_jpg_dir,f"{patient_id}_target.jpg"), dpi = DPI, transparent = True)
                plt.close()
...
def dcm2img(dcm, windowing = True, return_uint8 = True):
    ...
    if hasattr(dcm_data, "pixel_array"):
        ...
        if windowing: img = np.zeros([3, 512, 512])
        else: img =  np.zeros([1, 512, 512])
    if windowing: return windowing_brain(img - 1024, return_uint8 = return_uint8)
    else: return (img - 1024) / 2 ** 12

def dcm2np(dcm, windowing = True, return_uint8 = True):
    img = sitk.GetArrayFromImage(sitk.ReadImage(dcm))
    img = crop_center(img)
    
    if windowing: return windowing_brain(img, return_uint8 = return_uint8)
    else: return img / 2 ** 12
...
def sort_dcm(dcms, reverse = False):
    slices = {}
    for dcm in dcms:
        f = pydicom.dcmread(dcm, force = True)
        if hasattr(f, "SliceLocation"):
            slices[f.SliceLocation] = dcm
    sorted_dcms = [slices[slice_location] for slice_location in sorted(slices.keys())]
    if reverse:
        return sorted_dcms[::-1]
    else:
        return sorted_dcms


def get_scan(dcms, windowing = True, reverse = True, attrs = ["pixel_array", "SliceLocation", "PatientAge", "PatientSex"]):
    imgs, label = [], []
    for dcm in dcms:
        f = pydicom.dcmread(dcm, force = True)
        is_continue = False
        ...
        # stack image data
        img = f.pixel_array.astype(np.float)
        img = crop_center(np.expand_dims(img, 0))     
        if windowing: img = windowing_brain(img - 1024)
        else: img = (img - 1024) / 2 ** 12   
        imgs.append(img)
    
        # get patient meta data
        age = float(f.PatientAge[: -1]) * 0.01
        sex = 1.0 if f.PatientSex == "M" else 0.0
        label = [age, sex]
