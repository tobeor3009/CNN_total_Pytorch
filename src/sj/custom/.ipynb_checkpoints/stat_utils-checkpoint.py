...
def plt_auroc(nm_cases, ab_cases, results, png_name):
    for split in ["internal_validation", "external_validation"]:
        ...
        testy = np.concatenate((np.ones(len(total_ab_probs)),np.zeros(len(total_nm_probs))))
        probs = np.concatenate((np.array(total_ab_probs), np.array(total_nm_probs)))
        ...
        plt.legend(legend,
                    fontsize = 'xx-large',
                    loc = 'lower right')
        plt.savefig(os.path.join('figures',result_dir, split, metric, 'auroc', png_name))
        plt.close()
...
def calculate_opt_threshold(nm_cases,
    ...
    for k in range(k_fold):
        if k_fold == 1:
            ...
            # random 함수 바꿔야함
            ab_idxes = np.unique(np.random.randint(0, len(total_ab_probs), int(0.8 * len(total_ab_probs))))
            nm_idxes = np.unique(np.random.randint(0, len(total_nm_probs), int(0.8 * len(total_nm_probs))))
            if len(ab_idxes) == 0 or len(nm_idxes) == 0: continue
                      
            kfold_ab_probs = total_ab_probs[ab_idxes]
            kfold_nm_probs = total_nm_probs[nm_idxes]
            
            testy = np.concatenate([np.ones_like(kfold_ab_probs), np.zeros_like(kfold_nm_probs)])
...
#         indices = rng.randint(0, len(y_pred), len(y_pred))
    ...
    for label, scores in bootstrapped_scores.items():
        ...
        CI_scores[label] = np.array([score for score in scores if lower <= score <= upper])
...
#         indices = rng.randint(0, len(y_pred), len(y_pred))
    ...
    scores_in_ci = [score for score in bootstrapped_scores if lower <= score <= upper] 
    return scores_in_ci
...
def get_results(result_path, normal_stat, metric, step = -1, normalize = True, splits = ["internal_validation", "external_validation"]):
    ...
    for split in splits:
        for severity in severities:
            dirs = sorted(glob(os.path.join(result_path, split, severity, "*")))
            for dir in (dirs):    
...
#                         diff_score[i] /= pixel_spacing ** 2
                        if normalize:
                            ...
                            normalized_score = (diff_score[i] - mean) / (std + 1e-5) 
...
#                     score /= (pixel_spacing ** 2)
                    if i in normal_stat: normal_stat[i].append(score)
                    else: normal_stat[i] = [score]
