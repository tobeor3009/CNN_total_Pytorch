...
def gaussian_3d(window_size, sigma):
    ...
    gauss_3d = gauss_2d.unsqueeze(2) * gauss_1d.unsqueeze(0).unsqueeze(1)
    return gauss_3d


def create_window_3d(window_size, channel):
    _3D_window = gaussian_3d(window_size, 1.5).unsqueeze(0).unsqueeze(0)
...
def _ssim_3d(img1, img2, window, window_size, channel, size_average=True):
    ...
    sigma1_sq = F.conv3d(img1 * img1, window,
                         padding=window_size // 2, groups=channel) - mu1_sq
    sigma2_sq = F.conv3d(img2 * img2, window,
                         padding=window_size // 2, groups=channel) - mu2_sq
    sigma12 = F.conv3d(img1 * img2, window,
