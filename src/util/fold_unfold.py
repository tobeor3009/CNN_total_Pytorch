...
def unfold_nd(input_tensor, patch_size, stride, img_dim):
...
def get_fold_idx_tensor(batch_size, in_channels, output_size, patch_size, stride, img_dim, device):
    patch_per_dim = int((output_size - patch_size) / stride) + 1
    ...
    output_size_tuple = to_ntuple(output_size, img_dim)
    output_size_numel = get_list_numel(output_size_tuple)
    
    fold_idx = torch.arange(output_size_numel,
                            dtype=torch.int64, device=device).view(1, 1, *output_size_tuple)
    fold_idx = unfold_nd(fold_idx, patch_size, stride, img_dim)
...
def fold_nd(unfold_tensor, batch_size, output_size, patch_size, stride, img_dim, fold_idx=None):
    patch_per_dim = int((output_size - patch_size) / stride) + 1
    patch_size_tuple = to_ntuple(patch_size, img_dim)
    patch_per_dim_tuple = to_ntuple(patch_per_dim, img_dim)
    num_patch = get_list_numel(patch_per_dim_tuple)
    ...
    if fold_idx is None:
        fold_idx = get_fold_idx_tensor(batch_size, in_channels, output_size, patch_size, stride, img_dim, unfold_tensor.device)
    ...
    output_tensor = output_tensor.reshape(batch_size, in_channels, *output_size_tuple)
    return output_tensor

def extract_patch_tensor(input_tensor, patch_size, stride):
    ...
    patch_tensor = unfold_nd(input_tensor, patch_size, stride, img_dim)
    patch_tensor = patch_tensor.permute(*permute_dim)
    patch_tensor = patch_tensor.contiguous().view(-1, C, *patch_size_gen)
    return patch_tensor

def combine_region_voting_patches(unfold_tensor, batch_size, image_size, patch_size, stride, img_dim):
    assert_patch_stride_size(patch_size, stride)
    _, in_channels, *_ = unfold_tensor.shape

    unfold_idx_tensor = torch.ones_like(unfold_tensor)
    fold_idx = get_fold_idx_tensor(batch_size, in_channels, image_size, patch_size, stride, img_dim, unfold_tensor.device)
    recon_tensor = fold_nd(unfold_tensor, batch_size, image_size, patch_size, stride, img_dim, fold_idx=fold_idx)
    divisor = fold_nd(unfold_idx_tensor, batch_size, image_size, patch_size, stride, img_dim, fold_idx=fold_idx)
    
    return recon_tensor / divisor

def combine_region_voting_patches_with_patch_weights(unfold_tensor, batch_size, image_size, patch_size, stride, img_dim,
                                                     stride_weight=None):
    assert_patch_stride_size(patch_size, stride)
    _, in_channels, *_ = unfold_tensor.shape
    if img_dim == 2:
    ...
    patch_per_dim = int((image_size - patch_size) / stride) + 1
    num_patch = patch_per_dim ** img_dim
    device, dtype = unfold_tensor.device, unfold_tensor.dtype

    if stride_weight is None:
        stride_weight = compute_stride_weights(image_size, patch_size, stride, img_dim).to(device=device, dtype=dtype)
    ...
    fold_idx = get_fold_idx_tensor(batch_size, in_channels, image_size, patch_size, stride, img_dim, unfold_tensor.device)
    recon_tensor = fold_nd(unfold_tensor, batch_size, image_size, patch_size, stride, img_dim, fold_idx=fold_idx)
    divisor = fold_nd(stride_weight, batch_size, image_size, patch_size, stride, img_dim, fold_idx=fold_idx)
...
def compute_stride_weights(image_size, patch_size, stride, img_dim):
    patch_per_dim = int((image_size - patch_size) / stride) + 1
    num_patch = patch_per_dim ** img_dim
...
class PatchSplitModel(nn.Module):
    def __init__(self, model, image_size, split_num, stride_num=2, output_size=None, patch_combine_method="region_voting", img_dim=2):
        ...
        self.extract_patches = partial(extract_patch_tensor, patch_size=patch_size, stride=stride)
        if patch_combine_method == "region_voting":
            self.combine_region_voting_patches = partial(combine_region_voting_patches, image_size=output_size, 
                                                         patch_size=output_patch_size, stride=output_stride, img_dim=img_dim)
