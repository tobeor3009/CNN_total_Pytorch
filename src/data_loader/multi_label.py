...
class MultiDataset(BaseDataset):
    def __init__(self,
        ...
        self.label_policy_dict = label_policy_dict
    ...
    def __getitem__(self, i):
        ...
        if self.on_memory and self.is_data_ready:
            ...
            label_str = get_parent_dir_name(image_path, level=self.label_level)
            label_array = self.label_policy_dict[label_str]
        ...
        if (not self.on_memory) or (self.on_memory and self.is_data_ready):
            ...
            label_array = torch.as_tensor(label_array, dtype=self.dtype)
            mask_array = torch.as_tensor(mask_array, dtype=self.dtype)
        return image_array, label_array, mask_array
