...
class SwinDiffusionUnet(nn.Module):
    def __init__(self, img_size=512, patch_size=4,
        ...
        common_kwarg_dict = {"dim":embed_dim,
                            ...
                            "emb_dim_list":emb_dim_list + [embed_dim],
    ...
    def forward(self, x, time, cond=None, x_self_cond=None, class_labels=None,
        ...
        latent, hidden_1_list, hidden_2_list = self.latent_encoder(cond)
        ...
        skip_connect_list_1 = []
        skip_connect_list_2 = []
        for encode_layer_1, encode_layer_2, hidden_1, hidden_2 in zip(self.encode_layers_1, self.encode_layers_2,
                                                                        hidden_1_list, hidden_2_list):
            
            x = encode_layer_1(x, *emb_list, hidden_1)
            skip_connect_list_1.append(x)
            
            x = encode_layer_2(x, *emb_list, hidden_2)
            skip_connect_list_2.append(x)

        x = self.mid_layer(x, *emb_list)

        for skip_conv_layer_1, skip_conv_layer_2, decode_layer_1, decode_layer_2, hidden_1, hidden_2 in zip(self.skip_conv_layers_1, self.skip_conv_layers_2,
                                                                                                              self.decode_layers_1, self.decode_layers_2, 
                                                                                                              hidden_1_list[::-1], hidden_2_list[::-1]):
            skip_x = skip_connect_list_1.pop()
            x = skip_conv_layer_1(x, skip_x)
            x = decode_layer_1(x, *emb_list, hidden_1)

            skip_x = skip_connect_list_2.pop()
            x = skip_conv_layer_2(x, skip_x)
            x = decode_layer_2(x, *emb_list, hidden_2)
...
class SwinDiffusionEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for encode_layer_1, encode_layer_2 in zip(self.encode_layers_1, self.encode_layers_2):
            x = encode_layer_1(x)
            hidden_1_list.append(x)
            x = encode_layer_2(x)
            hidden_2_list.append(x)

        x = self.mid_layer(x)
        x = self.pool_layer(x)
        return x, hidden_1_list, hidden_2_list