...
class SwinDiffusionUnet(nn.Module):
    def __init__(self, img_size=512, patch_size=4,
        ...
        for i_layer in range(self.num_layers):
            ...
            encode_layer_2 = BasicLayerV2(dim=layer_dim * 2,
                                        ...
                                        downsample=None,
                                        **common_kwarg_dict)
            encode_attn = LinearAttention(dim=layer_dim * 2, num_heads=num_heads[i_layer], 
            ...
            self.encode_attns.append(encode_attn)
        ...
        for d_i_layer in range(self.num_layers, 0, -1):
            ...
            decode_attn = LinearAttention(dim=layer_dim // 2, num_heads=num_heads[i_layer],
                                    use_checkpoint=use_checkpoint[i_layer])
            ...
            self.decode_attns.append(decode_attn)
        ...
        for idx in range(last_conv_num):
            ...
            if idx == 1:
                final_conv = nn.Sequential(
                    ...
                    LinearAttention2D(dim=out_channel, num_heads=max(num_heads[i_layer] // 2, 1),
                                      use_checkpoint=use_checkpoint[i_layer])
                )
            else:
    ...
    def forward(self, x, time, cond=None, x_self_cond=None, class_labels=None,
        ...
        skip_connect_list_1 = []
        skip_connect_list_2 = []
        for idx, (encode_layer_1, encode_layer_2, encode_attn) in enumerate(zip(self.encode_layers_1,
        ...
        for idx, (skip_layer_1, skip_layer_2,
                  decode_layer_1, decode_layer_2, decode_attn) in enumerate(zip(self.skip_layers_1,
                                                                                self.skip_layers_2,
...
class SwinDiffusionEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for encode_layer_1, encode_layer_2, encode_attn in zip(self.encode_layers_1,
                                                                self.encode_layers_2,
                                                                self.encode_attns):
            x = encode_layer_1(x)
            x = encode_layer_2(x)
