...
class PatchExpanding(nn.Module):
    def __init__(self, input_resolution, dim,
                 return_vector=True, upsample_scale=2,
        ...
        self.pixel_shuffle_linear = nn.Conv2d(dim, dim * (upsample_scale ** 2) // 2,
                                              kernel_size=1, bias=False)
        self.pixel_shuffle = nn.PixelShuffle(upsample_scale)
        self.concat_linear = nn.Conv2d(dim // 2, dim // 2, bias=False,
                                       kernel_size=3, padding=1)
        self.norm_layer = norm_layer([dim // 2,
                                      input_resolution[0] * upsample_scale,
                                      input_resolution[1] * upsample_scale])
