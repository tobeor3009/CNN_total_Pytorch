...
from .swin_layers_diffusion_2d import BasicLayerV1, BasicLayerV2, SkipConv1D
...
class SwinDiffusionUnet(nn.Module):
    def __init__(self, img_size=512, patch_size=4,
                 in_chans=1, cond_chans=3, out_chans=1, out_act=None, num_class_embeds=None, cond_drop_prob=0.5,
        ...
        if self.cond_drop_prob > 0:
            self.r_null_class_emb = nn.Parameter(torch.randn(time_emb_dim))
            self.g_null_class_emb = nn.Parameter(torch.randn(time_emb_dim))
            self.b_null_class_emb = nn.Parameter(torch.randn(time_emb_dim))
        else:
    ...
    def forward(self, x, time, cond=None, x_self_cond=None, class_labels=None,
        ...
        skip_connect_list_1 = []
        skip_connect_list_2 = []
        for cond_encode_layer_1, cond_encode_layer_2, encode_layer_1, encode_layer_2 in zip(self.cond_encode_layers_1, self.cond_encode_layers_2, 
                                                                                                                           self.encode_layers_1, self.encode_layers_2):
        ...
        x = self.mid_layer(x, *emb_list, cond)
