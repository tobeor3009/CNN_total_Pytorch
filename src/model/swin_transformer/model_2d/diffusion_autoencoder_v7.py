...
from .swin_layers_diffusion_2d import LinearAttention2D, LinearAttention, Attention
...
class SwinDiffusionUnet(nn.Module):
    def __init__(self, img_size=512, patch_size=4,
        ...
        for d_i_layer in range(self.num_layers, 0, -1):
            ...
            skip_layer = SkipLinear(layer_dim * 2, layer_dim,
                                    norm=get_norm_layer_partial(num_heads[i_layer]))
            skip_attn = LinearAttention(dim=layer_dim, num_heads=num_heads[i_layer])
            ...
            self.skip_attns.append(skip_attn)
        ...
        for idx in range(last_conv_num):
            ...
            if idx == 0:
                final_conv = nn.Sequential(
                    ...
                    LinearAttention2D(dim=out_channel, num_heads=num_heads[i_layer])
                )
            else:
    ...
    def forward(self, x, time, cond=None, x_self_cond=None, class_labels=None,
        ...
        for idx, (skip_layer, skip_attn, decode_layer) in enumerate(zip(self.skip_layers, self.skip_attns, self.decode_layers)):
            if idx < 2:
                layer_emb_list = emb_list[:2]
            else:
                layer_emb_list = emb_list
