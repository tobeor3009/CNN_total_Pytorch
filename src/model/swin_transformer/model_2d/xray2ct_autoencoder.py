...
class SwinXrayCTAutoEncoder(nn.Module):
    def __init__(self, img_size=256, patch_size=4, cond_chans=16,
        ...
        for i_layer in range(self.num_layers):
            ...
            encode_layer = BasicLayerV2_3D(dim=layer_dim,
        ...
        self.mid_layer = BasicLayerV2_3D(dim=layer_dim,
        ...
        for i_layer in range(self.num_layers - 1, -1, -1):
            ...
            decode_layer = BasicLayerV2_3D(dim=layer_dim,
                                        input_resolution=feature_resolution,
                                        depth=depths[i_layer],
                                        num_heads=num_heads[i_layer],
                                        window_size=window_sizes[i_layer],
        ...
        self.seg_final_conv = Output3D(layer_dim // 2, out_chans, act=out_act)
...
class SwinDiffusionEncoder(nn.Module):
    def __init__(self, img_size=512, patch_size=4, in_chans=1, emb_chans=1024,
        ...
        for i_layer in range(self.num_layers):
            ...
            feature_resolution = np.array((patches_resolution[0] // (2 ** i_layer),
                                            patches_resolution[1] // (2 ** i_layer)))
            layer = BasicLayerV2(dim=layer_dim,
                                    input_resolution=feature_resolution,
                                    depth=depths[i_layer],
                                    num_heads=num_heads[i_layer],
                                    window_size=window_sizes[i_layer],
