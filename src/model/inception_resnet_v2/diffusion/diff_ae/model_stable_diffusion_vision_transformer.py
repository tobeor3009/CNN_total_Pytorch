...
BASE_ATTN_INFO = {"full_attn": "flash-attn", "num_heads": 8}
def get_attn_info(num_heads):
    attn_info = BASE_ATTN_INFO.copy()
    attn_info["num_heads"] = num_heads
    return attn_info

class VAE_Encoder(nn.Module):
    def __init__(self, in_channel=3, block_size=128, level=3, img_dim=2):
        super().__init__()
        self.in_channel = in_channel
        self.block_size = block_size
        self.norm = 
        self.init_conv = nn.Conv2d(in_channel, block_size, kernel_size=3, padding=1)
        max_block_size = block_size * 4
        conv_block_list = []
        common_conv_kwarg_dict = {
            "kernel_size": 3,
            "stride": 1,
            "padding": 1,
            "norm": "group",
        ...
        for depth_idx in range(level):
            input_block_size = min(block_size * (2 ** depth_idx), max_block_size)
            output_block_size = min(input_block_size * 2, max_block_size)
            conv_0 = ResNetBlockND(input_block_size, input_block_size, **common_conv_kwarg_dict)
            conv_1 = ResNetBlockND(input_block_size, output_block_size, **common_conv_kwarg_dict)
            conv_2 = nn.Conv2d(128, 128, kernel_size=3, stride=2, padding=0)

            module_list = [conv_0, conv_1, conv_2]
            if depth_idx == level - 1:
                conv_3 = nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=0)
                module_list.append(conv_3)

        final_conv_block = [ResNetBlockND(output_block_size, output_block_size, **common_conv_kwarg_dict)]
