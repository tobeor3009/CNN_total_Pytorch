...
class InceptionResNetV2_UNet(nn.Module):
    ...
    def encode_forward(self, x, time_emb, class_emb):
        ...
        for idx, (_, layer) in enumerate(self.stem.items()):
            stem = layer(stem, time_emb, class_emb)
            if idx in [0, 3, 6]:
                skip_connect_list.append(stem)
        # mixed_5b
        mixed_5b = self.process_encode_block(self.mixed_5b, stem, time_emb, class_emb)
        # block_35
        block_35 = self.process_inception_block(self.block_35_mixed, self.block_35_up,
                                                mixed_5b, time_emb, class_emb,
                                                self.block_scale_list[0])
        # mixed_6a: skip connect target
        mixed_6a = self.process_encode_block(self.mixed_6a, block_35, time_emb, class_emb)
        skip_connect_list.append(mixed_6a)
        # block_17
        block_17 = self.process_inception_block(self.block_17_mixed, self.block_17_up,
                                                               mixed_6a, time_emb, class_emb,
                                                               self.block_scale_list[1])
        # mixed_7a: skip connect target
        mixed_7a = self.process_encode_block(self.mixed_7a, block_17, time_emb, class_emb)
        skip_connect_list.append(mixed_7a)
        # block_8
        block_8 = self.process_inception_block(self.block_8_mixed, self.block_8_up,
                                                            mixed_7a, time_emb, class_emb,
                                                            self.block_scale_list[2])
        # final_output
        output = self.final_conv(block_8, time_emb, class_emb)

        return output, skip_connect_list[::-1]
    
    def process_encode_block(self, block, x, time_emb, class_emb):
        output = []
        for (_, layer_list) in block.items():
            output_part = x
            for layer in layer_list:
                output_part = layer(output_part, time_emb, class_emb)
            output.append(output_part)
        output = torch.cat(output, dim=1)
        return output

    def process_inception_block(self, block_mixed, block_up,
                                x, time_emb, class_emb, scale):
        for mixed, up_layer in zip(block_mixed, block_up):
            x_temp = self.process_encode_block(mixed, x, time_emb, class_emb)
            x_temp = up_layer(x_temp, time_emb, class_emb)

            x = x + x_temp * scale
            x = self.act_layer(x)
        return x
        
    def decode_forward(self, encode_feature, skip_connect_list, time_emb, class_emb):
        ...
        for decode_idx, (skip_conv, layer) in enumerate(zip(self.skip_conv_list, self.decoder_layer_list)):
            ...
            decode_feature = layer(decode_feature, time_emb, class_emb)
    ...
    def get_attn_info(self, emb_type_list, attn_info):
        ...
        if attn_info is None:
            ...
            return {"emb_type_list":emb_type_list, "num_heads": num_heads, "full_attn": attn_info}
    ...
    def get_encode_mixed_5b(self, emb_dim_list, emb_type_list):
        ...
        mixed_5b_branch_2_2 = ConvBlock2D(block_size * 6, block_size * 6, 3, norm=norm, act=act,
                                        emb_dim_list=emb_dim_list_set[5], attn_info=attn_info,
                                        use_checkpoint=use_checkpoint)
    ...
    def get_inception_block(self, emb_dim_list, emb_type_list, block_depth, get_block_fn):
        block_list = [get_block_fn(emb_dim_list, emb_type_list, idx==0)
    ...
    def get_inception_block_35(self, emb_dim_list, emb_type_list, is_first):
        block_size = self.block_size
        norm = self.norm
        act = self.act
        use_checkpoint = self.use_checkpoint
    ...
    def get_inception_block_17(self, emb_dim_list, emb_type_list, is_first):
        block_size = self.block_size
        norm = self.norm
        act = self.act
        use_checkpoint = self.use_checkpoint
    ...
    def get_inception_block_8(self, emb_dim_list, emb_type_list, is_first):
        block_size = self.block_size
        norm = self.norm
        act = self.act
        use_checkpoint = self.use_checkpoint
...
class InceptionResNetV2_Encoder(InceptionResNetV2_UNet):
    ...
    def encode_forward(self, x):
        ...
        for idx, (_, layer) in enumerate(self.stem.items()):
            stem = layer(stem)
        # mixed_5b
        mixed_5b = self.process_encode_block(self.mixed_5b, stem, None, None)
        # block_35
        block_35 = self.process_inception_block(self.block_35_mixed, self.block_35_up,
                                                mixed_5b, None, None,
                                                self.block_scale_list[0])
        # mixed_6a: skip connect target
        mixed_6a = self.process_encode_block(self.mixed_6a, block_35, None, None)
        # block_17
        block_17 = self.process_inception_block(self.block_17_mixed, self.block_17_up,
                                                               mixed_6a, None, None,
                                                               self.block_scale_list[1])
        # mixed_7a: skip connect target
        mixed_7a = self.process_encode_block(self.mixed_7a, block_17, None, None)
        # block_8
        block_8 = self.process_inception_block(self.block_8_mixed, self.block_8_up,
                                                            mixed_7a, None, None,
                                                            self.block_scale_list[2])
        # final_output
        output = self.final_conv(block_8, None, None)
