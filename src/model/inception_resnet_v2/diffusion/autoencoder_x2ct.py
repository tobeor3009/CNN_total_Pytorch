...
import numpy as np
import torch
from .autoencoder_2d_v6 import InceptionResNetV2_UNet
...
from .diffusion_layer_3d import ConvBlock3D, MultiDecoder3D, Output3D
from .diffusion_layer_3d import MaxPool3d, AvgPool3d
...
class InceptionResNetV2_X2CT(nn.Module):
    def __init__(self, in_channel, cond_channel, out_channel, img_size, block_size=16,
                 ...
                 norm="group", act="silu", last_act=None,
                 last_channel_ratio=1, self_condition=False,
                 use_checkpoint=False, attn_info_list=[False, False, False, False, True],
        ...
        self.mixed_7a = self.get_encode_mixed_7a(emb_dim_list, emb_type_list)
        # 10x block8 (Inception-ResNet-C block)
        self.block_8_mixed, self.block_8_up = self.get_inception_block(emb_dim_list, emb_type_list,
                                                                         block_depth_list[2],
                                                                         self.get_inception_block_8)

        # Final convolution block
        self.final_conv = ConvBlock3D(block_size * 130, block_size * 96 * last_channel_ratio, 3,
                                      norm=norm, act=act,
                                      emb_dim_list=emb_dim_list, attn_info=self.get_attn_info(emb_type_list, attn_info_list[4]),
        ...
        for decode_idx, skip_channel in enumerate(skip_channel_list):
            ...
            skip_conv = nn.Conv3d(skip_conv_in_channel, skip_conv_out_channel, 1, bias=False)

            decoder_layer = MultiDecoder3D(decode_in_channel, decode_out_channel,
        ...
        self.decode_final_conv = Output3D(decode_out_channel, out_channel, act=last_act)
    ...
    def encode_forward(self, x, *args):
        ...
        mixed_7a = self.process_encode_block(self.mixed_7a, block_17, *args)
        ...
        # final_output
        output = self.final_conv(block_8, *args)
    ...
    def get_encode_stem(self, emb_dim_list, emb_type_list):
        ...
        return nn.ModuleDict({
            'stem_layer_1': ConvBlock3D(in_channel, block_size * 2, 3,
            ...
            'stem_layer_2': ConvBlock3D(block_size * 2, block_size * 2, 3,
                                        ...
                                        use_checkpoint=use_checkpoint),
            'stem_layer_3': ConvBlock3D(block_size * 2, block_size * 4, 3, norm=norm, act=act,
                                        emb_dim_list=emb_dim_list, attn_info=get_attn_info(emb_type_list, attn_info_list[1]),
                                        use_checkpoint=use_checkpoint),
            'stem_layer_4': MaxPool3d(3, stride=2, padding=padding_3x3),
            'stem_layer_5': ConvBlock3D(block_size * 4, block_size * 5, 1, norm=norm, act=act,
                                        emb_dim_list=emb_dim_list, attn_info=get_attn_info(emb_type_list, None),
                                        use_checkpoint=use_checkpoint),
            'stem_layer_6': ConvBlock3D(block_size * 5, block_size * 12, 3,
                                        ...
                                        use_checkpoint=use_checkpoint),
            'stem_layer_7': MaxPool3d(3, stride=2, padding=padding_3x3)
        })
    ...
    def get_encode_mixed_5b(self, emb_dim_list, emb_type_list):
        ...
        mixed_5b_branch_0_0 = ConvBlock3D(block_size * 12, block_size * 6, 1,
                                        norm=norm, act=act,
                                        emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                        use_checkpoint=use_checkpoint)
        mixed_5b_branch_1_0 = ConvBlock3D(block_size * 12, block_size * 3, 1, norm=norm, act=act,
                                        emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                        use_checkpoint=use_checkpoint)
        mixed_5b_branch_1_1 = ConvBlock3D(block_size * 3, block_size * 4, 5, norm=norm, act=act,
                                        emb_dim_list=emb_dim_list, attn_info=attn_info,
                                        use_checkpoint=use_checkpoint)
        mixed_5b_branch_2_0 = ConvBlock3D(block_size * 12, block_size * 4, 1, norm=norm, act=act,
                                        emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                        use_checkpoint=use_checkpoint)
        mixed_5b_branch_2_1 = ConvBlock3D(block_size * 4, block_size * 6, 3, norm=norm, act=act,
                                        emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                        use_checkpoint=use_checkpoint)
        mixed_5b_branch_2_2 = ConvBlock3D(block_size * 6, block_size * 6, 3, norm=norm, act=act,
                                        emb_dim_list=emb_dim_list, attn_info=attn_info,
                                        use_checkpoint=use_checkpoint)
        mixed_5b_branch_pool_0 = AvgPool3d(3, stride=1, padding=1)
        mixed_5b_branch_pool_1 = ConvBlock3D(block_size * 12, block_size * 4, 1, norm=norm, act=act,
                                            emb_dim_list=emb_dim_list, attn_info=attn_info,
                                            use_checkpoint=use_checkpoint)
    ...
    def get_encode_mixed_6a(self, emb_dim_list, emb_type_list):
        ...
        mixed_6a_branch_0_0 = ConvBlock3D(block_size * 20, block_size * 24, 3,
                                        stride=2, padding=padding_3x3, norm=norm, act=act,
                                        emb_dim_list=emb_dim_list, attn_info=attn_info,
                                        use_checkpoint=use_checkpoint)
        mixed_6a_branch_1_1 = ConvBlock3D(block_size * 20, block_size * 16, 1, norm=norm, act=act,
                                        emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                        use_checkpoint=use_checkpoint)
        mixed_6a_branch_1_2 = ConvBlock3D(block_size * 16, block_size * 16, 3, norm=norm, act=act,
                                        emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                        use_checkpoint=use_checkpoint)
        mixed_6a_branch_1_3 = ConvBlock3D(block_size * 16, block_size * 24, 3,
                                        stride=2, padding=padding_3x3, norm=norm, act=act,
                                        emb_dim_list=emb_dim_list, attn_info=attn_info,
                                        use_checkpoint=use_checkpoint)
        mixed_6a_branch_pool_0 = MaxPool3d(3, stride=2, padding=padding_3x3)
    ...
    def get_encode_mixed_7a(self, emb_dim_list, emb_type_list):
        ...
        mixed_7a_branch_0_1 = ConvBlock3D(block_size * 68, block_size * 16, 1, norm=norm, act=act,
                                        emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                        use_checkpoint=use_checkpoint)
        mixed_7a_branch_0_2 = ConvBlock3D(block_size * 16, block_size * 24, 3, stride=2, padding=padding_3x3,
                                        norm=norm, act=act,
                                        emb_dim_list=emb_dim_list, attn_info=attn_info,
                                        use_checkpoint=use_checkpoint)
        mixed_7a_branch_1_1 = ConvBlock3D(block_size * 68, block_size * 16, 1, norm=norm, act=act,
                                        emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                        use_checkpoint=use_checkpoint)
        mixed_7a_branch_1_2 = ConvBlock3D(block_size * 16, block_size * 18, 3, stride=2, padding=padding_3x3,
                                        norm=norm, act=act,
                                        emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                        use_checkpoint=use_checkpoint)
        mixed_7a_branch_2_1 = ConvBlock3D(block_size * 68, block_size * 16, 1, norm=norm, act=act,
                                        emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                        use_checkpoint=use_checkpoint)
        mixed_7a_branch_2_2 = ConvBlock3D(block_size * 16, block_size * 18, 3, norm=norm, act=act,
                                        emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                        use_checkpoint=use_checkpoint)
        mixed_7a_branch_2_3 = ConvBlock3D(block_size * 18, block_size * 20, 3, stride=2,
                                        padding=padding_3x3, norm=norm, act=act,
                                        emb_dim_list=emb_dim_list, attn_info=attn_info,
                                        use_checkpoint=use_checkpoint)
        mixed_7a_branch_pool_0 = MaxPool3d(3, stride=2, padding=padding_3x3)
    ...
    def get_inception_block_35(self, emb_dim_list, emb_type_list):
        ...
        branch_0_0 = ConvBlock3D(in_channels, block_size * 2, 1,
                                norm=norm, act=act,
                                emb_dim_list=emb_dim_list, attn_info=attn_info,
                                use_checkpoint=use_checkpoint)
        branch_1_0 = ConvBlock3D(in_channels, block_size * 2, 1,
                                norm=norm, act=act,
                                emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                use_checkpoint=use_checkpoint)
        branch_1_1 = ConvBlock3D(block_size * 2, block_size * 2, 3,
                                norm=norm, act=act,
                                emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                use_checkpoint=use_checkpoint)
        branch_2_0 = ConvBlock3D(in_channels, block_size * 2, 1,
                                norm=norm, act=act, 
                                emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                use_checkpoint=use_checkpoint)
        branch_2_1 = ConvBlock3D(block_size * 2, block_size * 3, 3,
                                norm=norm, act=act,
                                emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                use_checkpoint=use_checkpoint)
        branch_2_2 = ConvBlock3D(block_size * 3, block_size * 4, 3,
                                norm=norm, act=act,
                                emb_dim_list=emb_dim_list, attn_info=attn_info,
                                use_checkpoint=use_checkpoint)
        up = ConvBlock3D(mixed_channel, in_channels, 1,
    ...
    def get_inception_block_17(self, emb_dim_list, emb_type_list):
        ...
        branch_0_0 = ConvBlock3D(in_channels, block_size * 12, 1,
                                norm=norm, act=act,
                                emb_dim_list=emb_dim_list, attn_info=attn_info,
                                use_checkpoint=use_checkpoint)
        branch_1_0 = ConvBlock3D(in_channels, block_size * 8, 1,
                                norm=norm, act=act,
                                emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                use_checkpoint=use_checkpoint)
        branch_1_1 = ConvBlock3D(block_size * 8, block_size * 10, [1, 1, 7],
                                norm=norm, act=act,
                                emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                use_checkpoint=use_checkpoint)
        branch_1_2 = ConvBlock3D(block_size * 10, block_size * 10, [1, 7, 1],
                                norm=norm, act=act,
                                emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                use_checkpoint=use_checkpoint)
        branch_1_3 = ConvBlock3D(block_size * 10, block_size * 12, [7, 1, 1],
                                norm=norm, act=act,
                                emb_dim_list=emb_dim_list, attn_info=attn_info,
                                use_checkpoint=use_checkpoint)
        up = ConvBlock3D(mixed_channel, in_channels, 1,
    ...
    def get_inception_block_8(self, emb_dim_list, emb_type_list):
        ...
        branch_0_0 = ConvBlock3D(in_channels, block_size * 12, 1,
                                ...
                                emb_dim_list=emb_dim_list, attn_info=attn_info,
                                use_checkpoint=use_checkpoint)
        branch_1_0 = ConvBlock3D(in_channels, block_size * 12, 1,
                                norm=norm, act=act,
                                emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                use_checkpoint=use_checkpoint)
        branch_1_1 = ConvBlock3D(block_size * 12, block_size * 14, [1, 1, 3],
                                norm=norm, act=act,
                                emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                use_checkpoint=use_checkpoint)
        branch_1_2 = ConvBlock3D(block_size * 14, block_size * 14, [1, 3, 1],
                                norm=norm, act=act,
                                emb_dim_list=emb_dim_list, attn_info=non_attn_info,
                                use_checkpoint=use_checkpoint)
        branch_1_3 = ConvBlock3D(block_size * 14, block_size * 16, [3, 1, 1],
                                norm=norm, act=act,
                                emb_dim_list=emb_dim_list, attn_info=attn_info,
                                use_checkpoint=use_checkpoint)
        up = ConvBlock3D(mixed_channel, in_channels, 1,
...
class InceptionResNetV2_Encoder(InceptionResNetV2_UNet):
    def __init__(self, in_channel, img_size, block_size=16, emb_channel=1024,
        ...
        self.block_17_mixed, self.block_17_up = self.get_inception_block(None, None,
                                                                         ...
                                                                         self.get_inception_block_17)
        # Mixed 7a (Reduction-B block)
        self.mixed_7a = self.get_encode_mixed_7a(None, None)
        # 10x block8 (Inception-ResNet-C block)
        self.block_8_mixed, self.block_8_up = self.get_inception_block(None, None,
                                                                         block_depth_list[2],
                                                                         self.get_inception_block_8)
    ...
    def encode_forward(self, x):
        ...
        # mixed_7a: skip connect target
        mixed_7a = self.process_encode_block(self.mixed_7a, block_17)
        # block_8
        block_8 = self.process_inception_block(self.block_8_mixed, self.block_8_up,
                                                            mixed_7a, self.block_scale_list[2])
        # final_output
