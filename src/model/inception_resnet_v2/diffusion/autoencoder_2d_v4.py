...
class InceptionResNetV2_UNet(nn.Module):
    def __init__(self, in_channel, cond_channel, out_channel, img_size, block_size=16,
                 ...
                 norm="group", act="silu", last_act=None, num_class_embed_list=[],
        ...
        assert isinstance(num_class_embed_list, list), "you need to assign positive int list to num_class_embed_list"
        ...
        emb_dim_list = [emb_dim] + [emb_dim for _ in range(len(num_class_embed_list))] + [emb_channel]
        emb_type_list = ["seq"] + ["seq" for _ in range(len(num_class_embed_list))] +  ["seq"]
        ...
        class_emb_layer_list = []
        for num_class_embed in num_class_embed_list:
            class_emb_layer = nn.Sequential(
                nn.Embedding(num_class_embed, emb_dim),
                nn.SiLU(),
                nn.Linear(emb_dim, emb_dim),
            )
            class_emb_layer_list.append(class_emb_layer)
        self.class_emb_layer_list = nn.ModuleList(class_emb_layer_list)
    ...
    def forward(self, x, time, cond=None, x_self_cond=None, class_labels=None):
        ...
        class_emb_list = [class_emb_layer(class_label).to(dtype=x.dtype)
                          for class_label, class_emb_layer in zip(class_labels, self.class_emb_layer_list)]
        
        encode_feature, skip_connect_list = self.encode_forward(x, time_emb, *class_emb_list, latent)

        decode_feature = self.decode_forward(encode_feature, skip_connect_list,
                                             time_emb, *class_emb_list, latent)
