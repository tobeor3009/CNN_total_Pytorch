...
class InceptionResNetV2_UNet(nn.Module):
    ...
    def get_inception_block(self, emb_dim_list, emb_type_list, block_depth, get_block_fn):
        block_list = [get_block_fn(emb_dim_list, emb_type_list, get_attn_info=False)
                      for _ in range(block_depth)]
        mixed_list, up_list = [list(item) for item in zip(*block_list)]
        attn_layer = get_block_fn(emb_dim_list, emb_type_list, get_attn_info=True)
        return nn.ModuleList(mixed_list), attn_layer, nn.ModuleList(up_list)

    def get_inception_block_35(self, emb_dim_list, emb_type_list, get_attn_info):
        ...
        if get_attn_info:
            if self.use_inception_block_attn:
                attn_layer = self.get_attn_layer(in_channels, self.num_head_list[layer_idx], self.attn_info_list[layer_idx])
            else:
    ...
    def get_inception_block_17(self, emb_dim_list, emb_type_list, get_attn_info):
    ...
    def get_inception_block_8(self, emb_dim_list, emb_type_list, get_attn_info):
        block_size = self.block_size
