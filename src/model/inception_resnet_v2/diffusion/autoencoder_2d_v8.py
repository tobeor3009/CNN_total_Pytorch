...
from .diffusion_layer import ConvBlock2D, MultiDecoder2D, Output2D, RMSNorm
from .diffusion_layer import LinearAttention, Attention, MaxPool2d, AvgPool2d, MultiInputSequential
from .diffusion_layer import default, prob_mask_like, LearnedSinusoidalPosEmb
...
class InceptionResNetV2_UNet(nn.Module):
    def __init__(self, in_channel, cond_channel, out_channel, img_size, block_size=16,
        ...
        self.latent_model = InceptionResNetV2_Encoder(in_channel=cond_channel, img_size=img_size, block_size=block_size,
                                                    ...
                                                    use_checkpoint=use_checkpoint, attn_info_list=attn_info_list, num_head_list=num_head_list,
        ...
        self.final_conv = ConvBlock2D(block_size * 130, block_size * 96 * last_channel_ratio, 3,
                                      ...
                                      attn_info=self.get_attn_info(attn_info_list[4], num_head_list[4]),
                                      use_checkpoint=use_checkpoint[4])
        ...
        for decode_idx, skip_channel in enumerate(skip_channel_list):
            ...
            decode_in_channel = skip_channel
            ...
            decoder_layer = MultiDecoder2D(decode_in_channel, decode_out_channel,
                                            ...
                                            emb_dim_list=decode_emb_dim_list, emb_type_list=emb_type_list,
                                            attn_info=attn_info, use_checkpoint=use_checkpoint[4 - decode_idx])
    ...
    def process_inception_block(self, block_mixed, block_attn, block_up,
                                x, scale, *args):
        for mixed, attn_layer, up_layer in zip(block_mixed, block_attn, block_up):
            x_temp = self.process_encode_block(mixed, attn_layer, x, *args)
    ...
    def get_attn_info(self, attn_info, num_heads=None):
        if attn_info is None:
            return None
        elif attn_info is False:
            return {"num_heads": num_heads, "full_attn": False}
        elif attn_info is True:
            return {"num_heads": num_heads, "full_attn": True}
    
    def get_attn_layer(self, dim, num_heads, use_full_attn):
        if use_full_attn:
            attn_class = Attention
        else:
            attn_class = LinearAttention
        return attn_class(dim=dim, num_heads=num_heads)
    ...
    def get_encode_stem(self, emb_dim_list, emb_type_list):
        ...
        return nn.ModuleDict({
            'stem_layer_1': ConvBlock2D(in_channel, block_size * 2, 3, stride=2, padding=padding_3x3, 
                                        attn_info=get_attn_info(attn_info_list[0], num_head_list[0]),
                                        use_checkpoint=use_checkpoint[0], **common_arg_dict),
            'stem_layer_2': ConvBlock2D(block_size * 2, block_size * 2, 3,
                                        attn_info=None, use_checkpoint=use_checkpoint[0], **common_arg_dict),
            'stem_layer_3': ConvBlock2D(block_size * 2, block_size * 4, 3,
                                        attn_info=get_attn_info(attn_info_list[1], num_head_list[1]),
                                        use_checkpoint=use_checkpoint[1], **common_arg_dict),
            'stem_layer_4': MaxPool2d(3, stride=2, padding=padding_3x3),
            'stem_layer_5': ConvBlock2D(block_size * 4, block_size * 5, 1, 
                                        attn_info=None, use_checkpoint=use_checkpoint[1], **common_arg_dict),
            'stem_layer_6': ConvBlock2D(block_size * 5, block_size * 12, 3,
                                        attn_info=get_attn_info(attn_info_list[2], num_head_list[2]),
                                        use_checkpoint=use_checkpoint[2], **common_arg_dict),
    ...
    def get_encode_mixed_5b(self, emb_dim_list, emb_type_list):
        ...
        attn_layer = self.get_attn_layer(block_size * 20, self.num_head_list[layer_idx], self.attn_info_list[layer_idx])
    ...
    def get_encode_mixed_6a(self, emb_dim_list, emb_type_list):
        ...
        attn_layer = self.get_attn_layer(block_size * 68, self.num_head_list[layer_idx], self.attn_info_list[layer_idx])
    ...
    def get_encode_mixed_7a(self, emb_dim_list, emb_type_list):
        ...
        attn_layer = self.get_attn_layer(block_size * 130, self.num_head_list[layer_idx], self.attn_info_list[layer_idx])
    ...
    def get_inception_block(self, emb_dim_list, emb_type_list, block_depth, get_block_fn):
        ...
        mixed_list, attn_list, up_list = [list(item) for item in zip(*block_list)]
        return nn.ModuleList(mixed_list), nn.ModuleList(attn_list), nn.ModuleList(up_list)
    ...
    def get_inception_block_35(self, emb_dim_list, emb_type_list):
        ...
        attn_layer = self.get_attn_layer(mixed_channel, self.num_head_list[layer_idx], self.attn_info_list[layer_idx])
...
class InceptionResNetV2_Encoder(InceptionResNetV2_UNet):
    ...
    def encode_forward(self, x):
        ...
        for layer in self.stem.values():
            stem = layer(stem)
        # mixed_5b
        mixed_5b = self.process_encode_block(self.mixed_5b, self.mixed_5b_attn, stem)
        # block_35
        block_35 = self.process_inception_block(self.block_35_mixed, self.block_35_attn, self.block_35_up,
                                                mixed_5b, self.block_scale_list[0])
        # mixed_6a: skip connect target
        mixed_6a = self.process_encode_block(self.mixed_6a, self.mixed_6a_attn, block_35)
        # block_17
        block_17 = self.process_inception_block(self.block_17_mixed, self.block_17_attn, self.block_17_up,
                                                               mixed_6a, self.block_scale_list[1])
        # mixed_7a: skip connect target
        mixed_7a = self.process_encode_block(self.mixed_7a, self.mixed_7a_attn, block_17)
        # block_8
        block_8 = self.process_inception_block(self.block_8_mixed, self.block_8_attn, self.block_8_up,
