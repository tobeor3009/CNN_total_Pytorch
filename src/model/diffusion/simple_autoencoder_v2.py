...
class DiffusionUnet(nn.Module):
    def __init__(self, img_shape, in_chans, cond_chans, out_chans, emb_chans,
        ...
        self.down_block_list, self.down_attn_list = self.get_encode_block_list()
        self.mid_block_1, self.mid_attn, self.mid_block_2 = self.get_mid_block_list()
        self.up_block_list, self.up_attn_list, self.up_list = self.get_decode_block_list()
    ...
    def forward(self, x, time, cond=None, x_self_cond=None, class_labels=None,
        ...
        x, skip_list = self.process_encode_block(x, *emb_list)
        x = self.process_mid_block(x, *emb_list)
        x = self.process_decode_block(skip_list, x, *emb_list)
    ...
    def process_encode_block(self, x, *emb_list):
        skip_list = []
        for down_block, down_attn, use_checkpoint in zip(self.down_block_list,
                                                        self.down_attn_list, self.use_checkpoint):
            
            x = down_block(x, *emb_list)
            skip_list.append(x)
    ...
    def process_decode_block(self, skip_list, x, *emb_list):
        for up_block, up_attn, up, use_checkpoint in zip(self.up_block_list,
                                                         self.up_attn_list, self.up_list, self.use_checkpoint[::-1]):
            
            skip_x = skip_list.pop()
            x = torch.cat([x, skip_x], dim=1)
            x = up_block(x, *emb_list)
    ...
    def get_encode_block_list(self):
        ...
        for encode_idx in range(self.layer_depth):
            ...
            down_block_list.append(down_block_1)
            down_attn_list.append(down_attn)
        return down_block_list, down_attn_list
    ...
    def get_decode_block_list(self):
        ...
        for decode_idx in range(self.layer_depth, 0, -1):
            ...
            up_block_1 = ConvBlock2D(in_block_size, out_block_size, **common_kwarg_dict)
            ...
            up_block_list.append(up_block_1)
            up_attn_list.append(up_attn)
            up_list.append(up)

        return up_block_list, up_attn_list, up_list
...
class DiffusionEncoder(DiffusionUnet):
    ...
    def forward(self, x):
        ...
        x, _ = self.process_encode_block(x)
